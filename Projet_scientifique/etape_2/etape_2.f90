module global
    ...
    Real :: dx, dt, CFL, Fo, vitesse_convection, coeff_diffusion, delta, x0, tf, t
...
contains
    ...
    subroutine solution_finale()
        ...
        do while (t < tf)
            ...
            if (dt > Fo * dx**2 / coeff_diffusion) then
                dt = Fo * dx**2 / coeff_diffusion
            end if
            t = t + dt

            do i = 1, nb_points_spatiaux-2          ! pas spatial
                dx = maillage_x(i) - maillage_x(i-1)
                tab_temp(i) = tab_n(i) * (1 - vitesse_convection*dt/dx - 2 * dt*coeff_diffusion/(dx**2)) &
                    + tab_n(i-1) * (vitesse_convection*dt/dx + dt*coeff_diffusion/(dx**2)) &
                    + tab_n(i+1) * (dt*coeff_diffusion/(dx**2))
            end do
            tab_temp(0) = U_0
    ...
    subroutine solution_analytique()
        ...
        do while (t < tf) ! pas temporel
            ...
            do i = 1, nb_points_spatiaux-2 !tableau allant de 0 à nb_points_spatiaux-1 sans les bornes
                tab_n(i) = delta/((2*coeff_diffusion*t + delta**2)**(0.5)) &
                    * exp(-((maillage_x(i) - x0 - vitesse_convection*t) &
                    /((2*coeff_diffusion*t + delta**2)**(0.5)))**2)
            end do
            tab_n(0) = U_0
            tab_n(nb_points_spatiaux-1) = U_L
            if (modulo(compteur_pas_t, pas_t_save) == 0) then
